...
static ssize_t SHOW_##name( struct class *class, struct class_attribute *attr,  char *buf ) {   \
   strcpy( buf, buf_##name );                                                   \
   printk( "read %d\n", strlen( buf ) );                                        \
   return strlen( buf );                                                        \
}                                                                               \
static ssize_t STORE_##name( struct class *class, struct class_attribute *attr,  const char *buf, size_t count ){   \
   printk( "write %d\n", count );                                               \
...
static ssize_t STORE_##name( struct class *class, const char *buf, size_t count ) { \
   printk( "write %d\n", count );                                               \
   strncpy( buf_##name, buf, count );                                           \
   buf_##name[ count ] = '\0';                                                  \
   return count;                                                                \
...
__ATTR( name, 0660, &SHOW_##name, &STORE_##name ) /// делать файл 0660
