...
void CALL_CONVERSION ModuleInfo::FillModuleReadedName ( FTS::BasesTypes::ShortString* OutReadedModuleName ){
    if ( OutReadedModuleName )
        snprintf ( OutReadedModuleName->Data, FTS::Const::SIZE_SHORT_STRING, "GstVideoGraber - получает видеоифнормацию от устройств" );
}

//==================================================================================================================//
void CALL_CONVERSION ModuleInfo::FillModuleInfo ( FTS::BasesTypes::FullString* OutModuleInfo ){
    if ( OutModuleInfo )
        snprintf ( OutModuleInfo->Data, FTS::Const::SIZE_FULL_STRING, "%s", "Реализует получение видеоинформации средствами gstreamer от видеоустройств" );
}
...
void CALL_CONVERSION ModuleInfo::FillExportingInterfacesDescriber ( FTS::Ifaces::IExportInterfaceInfoDescriber* OutDescriber ){
    if ( OutDescriber ) {
        ...
        snprintf ( iface_info.InterfaceId.Data, FTS::Const::SIZE_SHORT_STRING, "%s", VIDEO_RECEIVER_IFACE_ID );
        iface_info.MinorVersion = VIDEO_RECEIVER_IFACE_VERSION;
