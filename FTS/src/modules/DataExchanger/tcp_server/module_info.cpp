...
void CALL_CONVERSION ModuleInfo::FillModuleReadedName( FTS::BasesTypes::ShortString* OutReadedModuleName ){
    if ( OutReadedModuleName ) {
        snprintf( OutReadedModuleName->Data, FTS::Const::SIZE_SHORT_STRING, "Простой TCP-сервер");
    }
}
...
void CALL_CONVERSION ModuleInfo::FillModuleInfo( FTS::BasesTypes::FullString* OutModuleInfo ){
    if ( OutModuleInfo ) {
        snprintf( OutModuleInfo->Data, FTS::Const::SIZE_FULL_STRING, "%s", "Модуль организации обмена данными, служающий входящие TCP-соединения, передает данные во все активные соединения и получает также из всех");
    }
}

//==================================================================================================================//
unsigned int CALL_CONVERSION ModuleInfo::GetModuleType( void ){
    return FTS::ModuleTypes::HARD;
}
...
void CALL_CONVERSION ModuleInfo::FillExportingInterfacesDescriber( FTS::Ifaces::IExportInterfaceInfoDescriber* OutDescriber ){
    if ( OutDescriber ) {
        ...
        snprintf( iface_info.InterfaceId.Data, FTS::Const::SIZE_SHORT_STRING, "%s", DATA_EXCHANGE_IFACE_ID );
        iface_info.MinorVersion = DATA_EXCHANGE_IFACE_VERSION;
