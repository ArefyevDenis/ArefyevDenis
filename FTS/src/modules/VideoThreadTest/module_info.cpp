...
void CALL_CONVERSION ModuleInfo::FillModuleReadedName ( FTS::BasesTypes::ShortString* OutReadedModuleName ){
    if ( OutReadedModuleName )
        snprintf ( OutReadedModuleName->Data, FTS::Const::SIZE_SHORT_STRING, "VideoThreadTest - модуль тестировки видеопотока" );
}

//==================================================================================================================//
void CALL_CONVERSION ModuleInfo::FillModuleInfo ( FTS::BasesTypes::FullString* OutModuleInfo ){
    if ( OutModuleInfo )
        snprintf ( OutModuleInfo->Data, FTS::Const::SIZE_FULL_STRING, "%s", "VideoThreadTest - подгружает видеоресивер и видеосендер, организует потоковую передачу видео" );
}
...
void CALL_CONVERSION ModuleInfo::FillExportingInterfacesDescriber ( FTS::Ifaces::IExportInterfaceInfoDescriber* OutDescriber ){
    if ( OutDescriber ) {
        ...
        snprintf ( iface_info.InterfaceId.Data, FTS::Const::SIZE_SHORT_STRING, "%s", VIDEO_THREAD_TEST_IFACE_ID );
        iface_info.MinorVersion = VIDEO_THREAD_TEST_IFACE_VERSION;
